...
int main() {
    ...
    while (1) {
        ...
        int r = rand() % 3;
        if (r == 0)
            memcpy(piece, piece1, sizeof(piece1));
        else if (r == 1)
            memcpy(piece, piece2, sizeof(piece2));
        else
            memcpy(piece, piece3, sizeof(piece3));
    ...
    break;  // quitter la boucle si la pièce n’a pas pu être placée
}
        ...
        // Suppression des lignes pleines et mise à jour de la grille
        int lignes_supprimees = 0;
        while (check_full_line(grille) != 11) {
            lignes_supprimees++;
            int ligne = check_full_line(grille);
            down_line(grille, ligne);
            system(CLEAR);  // Nettoyage avant réaffichage de la grille
            printf("--- Tour %d ---\n", tour);
            display_grille(grille);  // Réafficher la grille mise à jour
        }
